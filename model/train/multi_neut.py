...
#        SimpleTrainer, SyncMultiGPUTrainerReplicated, SyncMultiGPUReplicatedBuilder,
#        _int_to_range)
from tensorpack.train.interface import (
        TrainConfig, launch_train_with_config)
...
#from ..sources import sty_net as E
from ..sources import gen_net as G
from ..sources import neut_net as N
from ..sources import disc_net as D
#from ..sources import map_net as M
...
class Dataset_encoder(Dataset):
    def __init__(
        ...
        self.files_sorted_by_speakers = dict()
        self.distributed_files_by_speakers = dict()
        self.batch_size = batch_size
        self.enrollment_size = enrollment_size

        self.incrementals_per_speakers = dict()

        for spk in speakers:
            spk_files = glob.glob(os.path.join(self.data_path, spk, ('*/' * (depth-1))+'*.wav'))
            spk_files = [f for f in spk_files if os.path.isfile(f[:-4] + '_f0.npy')]
            self.files_sorted_by_speakers[spk] = spk_files

            full = np.random.choice(spk_files, len(spk_files), replace=False)
            piece = [full] * self.enrollment_size
            piece = np.reshape(piece, (len(full), self.enrollment_size))
            self.distributed_files_by_speakers[spk] = piece

            self.incrementals_per_speakers[spk] = 0

        self.incremental = 0

    def get_input(self, spk):
        all_enrolls = self.distributed_files_by_speakers[spk]
        #n_enrolls = len(all_enrolls)
        i_spk = self.incrementals_per_speakers[spk]
        enrolls = all_enrolls[i_spk]
        self.incrementals_per_speakers[spk] = (i_spk + 1) % len(all_enrolls)

        f0s, mel_lengths, _, arr_begins, _ = list(zip(*map(lambda f: self._get_array_(f[:-4] + '_f0.npy', array_size=self.meltime_size), enrolls)))
        sigs, sig_lengths, _, _, _ = list(zip(*map(lambda f, begin: self._get_signal_(f, array_size=self.meltime_size, array_begin=begin), enrolls, arr_begins)))

        #f0s = list(map(lambda s: self._f0_torchcrepe(s), signals))

        return np.asarray(sigs), np.asarray(sig_lengths), np.asarray(f0s), np.asarray(mel_lengths), spk, self.spk_to_lb[spk]
...
class MakeGetOperationFunction():
    ...
    def _make_get_op_fn(input, class_, get_cost_fn, get_opt_fn):
        ...
        def get_op_fn():
            ...
            for cost in costs:
                assert isinstance(cost, tf.Tensor) and cost.shape.ndims == 0
            ...
            for n, modules in enumerate(train_modules):
                ...
                grads_ = FilterNoneGrad().process(opt.compute_gradients(
                    costs[n], var_list=vars_, **common_kwargs))
...
class Single_encoder(Single_GPU_Trainer, MakeGetOperationFunction):
    # 1 * 2(optional, can be 1 when no moving updates)
    # * N_update_flow * N_mini_batch * ops
    def _make_get_op_fn(self, input, get_cost_fn, get_opt_fn):
        return MakeGetOperationFunction._make_get_op_fn(
                input, self, get_cost_fn, get_opt_fn)

class Multi_encoder(Batch_Expand_Multi_GPU_Trainer, MakeGetOperationFunction):
    # N_towers * 2(optional, can be 1 when no moving updates)
    # * N_update_flow * N_mini_batch * ops
    def _make_get_op_fn(self, input, get_cost_fn, get_opt_fn):
        return MakeGetOperationFunction._make_get_op_fn(
                input, self, get_cost_fn, get_opt_fn)
...
class Solver_encoder(Solver):
    ...
    def build_graph(
        ...
        # TODO 예전처럼 그냥 바로 붙이기
        reviewers_['disc_real'] = loss_disc_r #* grad_ratio
        reviewers_['disc_fake'] = loss_disc_f #* grad_ratio

        """
        grad_l1_norm_neut = tf.stop_gradient(loss_neut_pos + loss_neut_neg)
        grad_l1_norm_disc = tf.stop_gradient(loss_disc_r + loss_disc_f)

        to_neut = tf.cast(grad_l1_norm_neut > grad_l1_norm_disc, dtype=tf.float32)
        frac_n_d, frac_d_n = grad_l1_norm_neut / grad_l1_norm_disc, grad_l1_norm_disc / grad_l1_norm_neut

        neut_weight = (1 - to_neut) + (to_neut * frac_d_n)
        disc_weight = to_neut + ((1 - to_neut) * frac_n_d)

        reviewers_['deep_pos'] = neut_weight * loss_neut_pos
        reviewers_['deep_neg'] = neut_weight * loss_neut_neg

        reviewers_['disc_real'] = disc_weight * loss_disc_r
        reviewers_['disc_fake'] = disc_weight * loss_disc_f
        """
        ...
        Deep_Feat_Match = self.feature_matching(
                ...
                weight=0.1 + 0.1 * tf.minimum(tf.cast(self.global_step, dtype=tf.float32) / 100000, 1.0)
                )
