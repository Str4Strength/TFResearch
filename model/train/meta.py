...
class Batch_Expand_Multi_GPU_Graph_Builder(SyncMultiGPUReplicatedBuilder):
    ...
    def get_post_init_ops():
        ...
        def log_failure(name, reason):
            ...
            assert name not in trainable_names, "The aforementioned variable is trainable, so this is probably a fatal error."
            logger.warn(
        ...
        for v in all_vars:
            if not v.name.startswith('tower'): continue

            if v.name.startswith('tower0'):
                log_failure(v.name, "Name should not have prefix 'tower0' in this trainer!")
                # TODO some vars may still startswith tower0
                continue

            split_name = v.name.split('/')
            prefix = split_name[0]
            realname = '/'.join(split_name[2:])
            ...
            if exists(copy_from):
                ...
                log_failure(v.name, "Cannot fine {} in the graph!".format(realname))
