...
def softdtw_tf(D, M, gamma, w = 0.):
    assert w == 0., 'Not implemented: penalty w'

    B, N = shape(D)[0], shape(D)[-1]

    C = tf.cumsum(D, axis=2)
    R = [tf.pad(tf.ones([B, N]) * np.inf, ((0, 0), (1, 0)))]

    for i in range(M):
        C_k = C[:, i]
        C_k_back_pad = tf.pad(C_k[:, :-1], ((0, 0), (1, 0)))
        R_k_back, R_k_front = R[i, :-1], R[i, 1:]
        delta = tf.reduce_logsumexp([(C_k_back_pad - R_k_back) / gamma, (C_k_back_pad - R_k_front) / gamma], axis = 0)
        cumulative = tf.math.cumulative_logsumexp(delta, axis = 1)
        R_new = tf.pad(C_k - gamma * cumulative, ((0, 0), (1, 0)), constant_values = np.inf)
        R.append(R_new)

    R = tf.stack(R, axis = 1)
    R = tf.pad(R, ((0, 0), (0, 1), (0, 1)), constant_values = np.inf)

    return R


def softdtw_tfscan(D, gamma, w = 0.):
    assert w == 0., 'Not implemented: penalty w'

    B, M, N = shape(D)[0], shape(D)[1], shape(D)[2]
    ...
    def body(R_k, k):
        C_k = C[:, k]
        C_k_back_pad = tf.pad(C_k[:, :-1], ((0, 0), (1, 0)))
        R_k_back, R_k_front = R_k[:, :-1], R_k[:, 1:]
        delta = tf.reduce_logsumexp([(C_k_back_pad - R_k_back) / gamma, (C_k_back_pad - R_k_front) / gamma], axis = 0)
        cumulative = tf.math.cumulative_logsumexp(delta, axis = 1)
        cumulative = tf.pad(C_k - gamma * cumulative, ((0, 0), (1, 0)), constant_values = np.inf)
        return cumulative

    R_new = tf.scan(body, tf.range(M), R)
    R_new = tf.transpose(R_new, [1, 2, 0])

    R = tf.concat([R[:, :, None], R_new], axis = 2)
...
@jit(nopython = True)
def compute_softdtw_backward(D_, R, x_size, y_size, gamma, w = 0.):
    #B, N, M = shape(D_)[:3]
    B, N, M = D_.shape[:3]
    D = np.zeros(shape = (B, N + 2, M + 2), dtype = np.float32)
    E = np.full(shape = (B, N + 2, M + 2), fill_value = -np.inf, dtype = np.float32)
    D[:, 1 : N + 1, 1 : M + 1] = D_

    for k in range(B):
        R[k, x_size[k] + 1:, :] = -np.inf
        R[k, :, y_size[k] + 1:] = -np.inf
        E[k, x_size[k] + 1, y_size[k] + 1] = 0.
        R[k, x_size[k] + 1, y_size[k] + 1] = R[k, x_size[k], y_size[k]]

        for j in range(M + 1, 0, -1):
            for i in range(N + 1, 0, -1):
                if j >= y_size[k] + 1 or i >= x_size[k] + 1: continue

                a = E[k    , i + 1, j    ] + (R[k    , i + 1, j    ] - R[k    , i    , j    ] - D[k    , i + 1, j    ] - w) / gamma
                b = E[k    , i    , j + 1] + (R[k    , i    , j + 1] - R[k    , i    , j    ] - D[k    , i    , j + 1] - w) / gamma
                c = E[k    , i + 1, j + 1] + (R[k    , i + 1, j + 1] - R[k    , i    , j    ] - D[k    , i + 1, j + 1]    ) / gamma

                m = np.maximum(np.maximum(a, b), c)
                E[k, i, j] = m + np.log(np.exp(a - m) + np.exp(b - m) + np.exp(c - m))

    return E[:, 1 : N + 1, 1 : M + 1].astype(np.float32)
