...
def quantization_noise(
    ...
    if not is_conv:
        assert np.prod(in_features) % blocks == 0
    else:
    ...
    if is_conv and k != 1:
        ...
        mask = tf.greater(tf.random.uniform(shape=[np.prod(in_features) // blocks, *out_features], maxval=1, dtype=weight.dtype), p)
        mask = tf.reshape(tf.tile(mask[:, None], [1, blocks, * (1,) * len(out_features)]), features)
